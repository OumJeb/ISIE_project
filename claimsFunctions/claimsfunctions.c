...
#include "claimsfunctions.h"
    ...
    if(f!=NULL)
    {
        fprintf(f,"%s %d %d %d/%d/%d %d %s\n",cl.id,cl.type,cl.electoral_list,cl.date.day,cl.date.month,cl.date.year,cl.municipality,cl.text);
        fclose(f);
        return 1;
    }
    else return 0;
...
int modify_claims( char * filename, char id[20],Claims cnew)
{
    ...
    if (f!=NULL && f2!=NULL)
    {
        while(fscanf(f,"%s %d %d %d/%d/%d %d %s\n",cl.id,cl.type,cl.electoral_list,cl.date.day,cl.date.month,cl.date.year,cl.municipality,cl.text)!=EOF)
        {
            ...
            {
                fprintf(f2,"%s %d %d %d/%d/%d %d %s\n",cnew.id,cl.type,cnew.electoral_list,cnew.date.day,cnew.date.month,cnew.date.year,cnew.municipality,cnew.text);
                v=1;
            }
            else
            {
