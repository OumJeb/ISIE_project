...
int add_electoral(electoral e, char * liselect )
{
    ...
    if(f!=NULL)
    {
        fprintf(f,"%d %s %s %d %d %d %d %d %d %d \n",e.id_liste,e.orientation,e.id_HOL,e.id1,e.id2,e.id3,e.da.d,e.da.m,e.da.y);
        fclose(f);
        return 1;
    }
    else return 0;
...
int modify_electoral(int id_HOL, electoral new_e, char * liselect)
{
    ...
    {
        while(fscanf(f,"%s %s %s %s %s %s %s %d %d %d \n",&e.id_liste,e.orientation,&e.id_HOL,&e.id1,&e.id2,&e.id3,&e.da.d,&e.da.m,&e.da.y)!=EOF)
        {
            if(e.id_HOL!=id_HOL)
                fprintf(f2,"%s %s %s %s %s %s %s %d %d %d\n",e.id_liste,e.orientation,e.id_HOL,e.id1,e.id2,e.id3,e.da.d,e.da.m,e.da.y);
            else

                fprintf(f2,"%s %s %s %s %s %s %s %d %d %d\n",new_e.id_liste,new_e.orientation,new_e.id_HOL,new_e.id1,new_e.id2,new_e.id3,new_e.da.d,new_e.da.m,new_e.da.y);
...
electoral search_eletoral(int id_liste, char * liselect)
{
    ...
    if(f!=NULL )
    {
        while(fscanf(f,"%s %s %s %d %d %d %d %d %d %d\n",e.id_liste,e.orientation,e.id_HOL,e.id1,e.id2,e.id3,e.da.d,e.da.m,e.da.y)!=EOF && tr==0)
        {
